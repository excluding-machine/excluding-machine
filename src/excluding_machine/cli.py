...
def options_and_arguments():
    ...
    parser = argparse.ArgumentParser(
        description="Excluding-Machine thinks for you about the meanings.",
        epilog="Example:  excluding-machine input_text.txt > output_text.txt"
...
def run():
    """
    $ text | excluding-machine                        # Accepts text from the pipe
    $ echo "...<text>..." | excluding-machine         #

    $ excluding-machine multilogue.txt new_turn.txt    # ...or files.
    """
    ...
    try:
        ...
        syslog.openlog(
            ident="excluding-machine",
            logoption=syslog.LOG_NDELAY,
            facility=syslog.LOG_USER
        )
        # Signal (single line less than 4096 only!)
        syslog.syslog(syslog.LOG_INFO, f"excluding-machine: {utterance}.")
        syslog.closelog()
